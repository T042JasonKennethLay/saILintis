...
export function RestockPage() {
  ...
  const [activeItem, setActiveItem] = useState("Restock Approvals");
  ...
  const handleResolveRequest = async (approve: boolean) => {
    ...
    try {
      ...
      setSuccessMsg(`Restock request successfully ${approve ? "approved" : "rejected"}.`);
  ...
  const historyRequests = restockRequests.filter(r => r.status !== "Pending");
    ...
    return (
      ...
      >
        <div className="restock-access-denied">
          ...
          <h2>Access Denied</h2>
          <p>You do not have permission to view the Restock Approvals. Redirecting to dashboard...</p>
    ...
    >
      <div className="restock-container">
        <div className="restock-header">
          <h1 className="restock-title">Restock Approvals Console</h1>
          <p className="restock-desc">
            Review restock requests submitted by Restaurant Managers and approve them to be sent to suppliers.
          </p>
        </div>
        ...
        <div className="payroll-tabs">
          ...
          >
            <i className="fa-solid fa-hourglass-half" /> Pending Approvals ({pendingRequests.length})
          </button>
          <button
            className={`payroll-tab-btn ${activeTab === "history" ? "active" : ""}`}
            onClick={() => { setActiveTab("history"); setSelectedRequest(null); }}
          >
            <i className="fa-solid fa-clock-rotate-left" /> History Ledger ({historyRequests.length})
          </button>
        </div>
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="restock-table-empty">
                        {activeTab === "pending" ? "No pending restock requests." : "No request history."}
                      </td>
                    </tr>
                  ) : (
                    currentList.map((req) => (
            ...
            <div className="restock-modal-card">
              <div className="restock-modal-header">
                <div className="restock-modal-title-group">
                  <span className="restock-modal-subtitle">Restock Request Review</span>
              ...
              <div className="restock-modal-body">
                  ...
                  <div className="restock-info-item" style={{ gridColumn: "span 2" }}>
                    ...
                    <div>
                      <span className={`rest-badge rest-badge-${selectedRequest.status.toLowerCase().replace(/\s+/g, "-")}`}>
                        {selectedRequest.status === "Approved" ? "ACC" : (selectedRequest.status === "Passed to Supplier" ? "Sent to Supplier" : selectedRequest.status)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {selectedRequest.status === "Pending" ? (
                  <div className="restock-action-buttons">
                    <button
                      className="restock-btn-reject"
                      disabled={submitting}
                      onClick={() => handleResolveRequest(false)}
                    >
                      <i className="fa-solid fa-circle-xmark" /> Reject Request
                    </button>
                    <button
                    ...
                    >
                      <i className="fa-solid fa-circle-check" /> Approve Request
                    </button>
                  </div>
                ) : (
                  <div className="restock-resolution-summary">
                    <h4 className="restock-resolution-title">Resolution Summary</h4>
                    <div className="restock-resolution-grid">
                      <div className="restock-resolution-col">
                        <span className="restock-info-label">Decision</span>
